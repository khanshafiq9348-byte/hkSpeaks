...
import { Mic2, Lock, Mail, User as UserIcon, Loader2, AlertCircle } from "lucide-react";
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiClient<{ access_token: string; user: any }>("/auth/signup", {
        method: "POST",
  ...
  return (
    <div className="min-h-screen bg-[#090A0F] flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <p className="mt-1 text-xs text-gray-400">Get 10,000 free monthly characters instantly</p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-[#12141F] py-8 px-6 shadow-2xl border border-[#202436] sm:rounded-2xl sm:px-10">
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-300">Name or Studio Name</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-300">Password (min 8 characters)</label>
              <div className="mt-1 relative">
